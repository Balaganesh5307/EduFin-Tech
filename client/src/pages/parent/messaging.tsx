...
export const ParentMessaging: React.FC = () => {
  ...
  const loadConversations = async () => {
    try {
      ...
      console.warn('Failed retrieving parent conversations:', err);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="space-y-6 text-left">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">Real-time chats and consultation channels with your child's course instructors.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch h-[60vh]">
        ...
        <div className="lg:col-span-4 glass-panel border-slate-900 rounded-3xl p-4 flex flex-col space-y-3 overflow-y-auto">
          <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest block pl-2 mb-2">Faculty Rooms</span>
          {conversations.length === 0 ? (
            <p className="text-slate-500 text-xs italic pl-2">No active consult rooms.</p>
          ) : (
            conversations.map((t) => {
          ...
          {selectedThread ? (
            <>
              <div className="border-b border-slate-900 pb-3 text-left">
                ...
                <span className="text-[9px] text-slate-500 block uppercase font-bold tracking-wider">Faculty Instructor</span>
              </div>
            ...
            <div className="h-full flex items-center justify-center text-slate-500 italic text-xs">Select a consult room to display message trails.</div>
          )}
        </div>
      </div>
    </div>
