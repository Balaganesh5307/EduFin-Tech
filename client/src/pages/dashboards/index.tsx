...
export const DashboardsIndex: React.FC = () => {
  ...
  const renderParent = () => {
    const parentData = data || {
      child: { name: "Alex Johnson", rollNumber: "2024-CSE-084", course: "B.Tech CSE", semester: "Semester 5" },
      summary: { pendingFees: 20000, paidFees: 90000, attendanceRate: 91.2, gpa: 8.7 },
      invoices: [
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="glass-panel border-slate-900 rounded-2xl p-6 flex flex-col sm:flex-row items-center gap-4 text-left">
          ...
          <div>
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Student Profile Registered</span>
            <h3 className="text-xl font-bold text-slate-200 mt-0.5">{parentData.child?.name}</h3>
            <p className="text-xs text-slate-400 mt-1">
              Roll: <strong>{parentData.child?.rollNumber}</strong> | Course: <strong>{parentData.child?.course}</strong> | {parentData.child?.semester}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Outstanding Dues</span>
            <h4 className="text-2xl font-bold text-slate-100">₹{parentData.summary?.pendingFees.toLocaleString()}</h4>
            <span className="text-[10px] text-amber-500 font-semibold block pt-2">Invoice due in 22 days</span>
          </div>
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Total Fees Settled</span>
            <h4 className="text-2xl font-bold text-slate-100">₹{parentData.summary?.paidFees.toLocaleString()}</h4>
            <span className="text-[10px] text-emerald-400 font-semibold block pt-2">100% of Sem 4 cleared</span>
          </div>
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Current Attendance</span>
            <h4 className="text-2xl font-bold text-slate-100">{parentData.summary?.attendanceRate}%</h4>
            <span className="text-[10px] text-emerald-400 font-semibold block pt-2">Regular class attendance</span>
          </div>
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Current CGPA</span>
            <h4 className="text-2xl font-bold text-slate-100">{parentData.summary?.gpa}</h4>
            <span className="text-[10px] text-indigo-400 font-semibold block pt-2">Top 15% class percentile</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="glass-panel border-slate-900 rounded-2xl p-6 md:col-span-6 space-y-4 text-left">
            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <CreditCard className="h-4.5 w-4.5 text-indigo-400" /> Pending Invoices
            </h4>
            <div className="space-y-3.5">
              {parentData.invoices?.map((inv: any) => (
                <div key={inv.id} className="p-4 rounded-xl border border-slate-900 bg-slate-950/20 flex justify-between items-center text-xs">
                  <div>
                    <h5 className="font-semibold text-slate-200">{inv.name}</h5>
                    <span className="text-[10px] text-slate-500">Status: {inv.status}</span>
                  </div>
                  <div className="text-right">
                    <span className="font-bold text-slate-200 block mb-1.5">₹{inv.amount.toLocaleString()}</span>
                    {inv.status === 'Unpaid' ? (
                      <button
                        onClick={() => triggerPayment(inv)}
                        className="bg-indigo-600 hover:bg-indigo-500 text-white font-semibold px-3 py-1.5 rounded-lg hover:shadow-md transition-all text-[11px] active:scale-95"
                      >
                        Pay Online
                      </button>
                    ) : (
          ...
          {/* Academic GPA chart curve */}
          <div className="glass-panel border-slate-900 rounded-2xl p-6 md:col-span-6 space-y-4 text-left">
            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <TrendingUp className="h-4.5 w-4.5 text-indigo-400" /> GPA Progress Curve
            </h4>
            <div className="h-56">
  ...
  const renderFaculty = () => {
    const facultyData = data || {
      summary: { assignedClasses: 4, totalStudents: 142, averageAttendance: 84.6, pendingReports: 2 },
      timetable: [
    ...
    const students = [
      { id: 's1', name: 'Alice Smith', roll: '2024-CSE-003', attendance: '94%' },
      { id: 's2', name: 'Bob Jenkins', roll: '2024-CSE-012', attendance: '88%' },
      { id: 's3', name: 'Charlie Dave', roll: '2024-CSE-023', attendance: '72%' }
    ];
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Assigned Subject Batches</span>
            <h4 className="text-2xl font-bold text-slate-100">{facultyData.summary?.assignedClasses} Classrooms</h4>
          </div>
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Students Registered</span>
            <h4 className="text-2xl font-bold text-slate-100">{facultyData.summary?.totalStudents} Active</h4>
          </div>
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Attendance Average</span>
            <h4 className="text-2xl font-bold text-slate-100">{facultyData.summary?.averageAttendance}%</h4>
          </div>
          <div className="glass-panel border-slate-900 p-5 rounded-2xl text-left space-y-1">
            <span className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">Pending Progress Reports</span>
            <h4 className="text-2xl font-bold text-slate-100">{facultyData.summary?.pendingReports} Incomplete</h4>
          </div>
        </div>

        {/* Timetable & Mark Attendance bento */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          
          {/* Daily Schedule List */}
          <div className="glass-panel border-slate-900 rounded-2xl p-6 md:col-span-5 space-y-4 text-left">
            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <Calendar className="h-4.5 w-4.5 text-indigo-400" /> Today's Lecture Timetable
            </h4>
            <div className="space-y-3.5">
              {facultyData.timetable?.map((timeSlot: any, index: number) => (
                <div key={index} className="p-4 rounded-xl border border-slate-900 bg-slate-950/20 flex gap-4 text-xs">
                  <div className="text-indigo-400 shrink-0 pt-0.5">
                    <Clock className="h-4.5 w-4.5" />
                  </div>
                  <div className="space-y-1">
                    <h5 className="font-semibold text-slate-200">{timeSlot.course}</h5>
                    <p className="text-[10px] text-slate-500">
                      Time: <strong>{timeSlot.time}</strong> | Room: <strong>{timeSlot.room}</strong> ({timeSlot.type})
                    </p>
                  </div>
                </div>
              ))}
          ...
          {/* Quick Mark attendance portal sheet */}
          <div className="glass-panel border-slate-900 rounded-2xl p-6 md:col-span-7 space-y-4 text-left">
            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              <Calendar className="h-4.5 w-4.5 text-indigo-400" /> Attendance Ledger Sheet (B.Tech CS - A)
            </h4>
            <div className="divide-y divide-slate-900/60">
              {students.map((stu) => (
                <div key={stu.id} className="py-3.5 flex items-center justify-between first:pt-0 last:pb-0">
                  <div>
                    <h5 className="text-sm font-bold text-slate-200">{stu.name}</h5>
                    <span className="text-[10px] text-slate-500">ID: {stu.roll} | Previous attendance: {stu.attendance}</span>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => markAttendance(stu.id, 'Present')}
                      className={`text-xs px-3 py-1.5 rounded-lg border font-semibold transition-all ${
                        markedAttendance[stu.id] === 'Present'
                    ...
                    <button
                      ...
                      className={`text-xs px-3 py-1.5 rounded-lg border font-semibold transition-all ${
                        markedAttendance[stu.id] === 'Absent'
